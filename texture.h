...
class image_texture : public texture {
    public:
        image_texture(const char* filename) : image(filename) {
            missing_tex = make_shared<checker_texture>(0.5,color(0.96875,0,0.96875), color(0,0,0));
        }

        color value(double u, double v, const point3& p) const override {
            if (image.height() <= 0) return missing_tex->value(u,v,p);
    ...
    private:
        shared_ptr<texture> missing_tex;
        rtw_image image;
};
